...
//顶部ScrollView高度
static const CGFloat SegmentHeight = 50.0f;
...
- (void)setIsBounseEaqual:(BOOL)isBounseEaqual {
    _isBounseEaqual = isBounseEaqual;
    _segment.isBounseEaqual = isBounseEaqual;
}
...
- (void)setScrollEnable:(BOOL)scrollEnable {
    ...
    _segment.collectionView.userInteractionEnabled = scrollEnable;
}
...
- (void)showUnDoneNum:(NSInteger)num index:(NSInteger )index {
    if (_titles.count > index) {
        SlideModel *model = _titles[index];
        model.num = num;
...
-(void)showIconIndex:(NSInteger)index {
    [self showIcon:@"SliderSwitchIcon" index:index];
}
